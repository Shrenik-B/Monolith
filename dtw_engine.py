...
class DTWEngine:
    ...
    def find_similar_trajectories(
        ...
        # Slide rolling historical window across full dataset
        for start_idx in range(0, n_samples - hist_window_len + 1, self.step_size):
            end_idx = start_idx + hist_window_len

            # Strictly restrict historical matching candidates to periods PRIOR to the query window start date (no future lookahead)
            ...
            # Forward market return following the historical period (what happened after)
            forward_return = None
            if spy_returns is not None and end_idx + 30 <= n_samples:
                # Cumulative return in next 30 trading days
        ...
        # Sort matches by lowest normalized DTW distance
