...
def save_model_and_metadata(
    ...
    """
    # 1. Save primary model binary and timestamped backup (#9)
    with open(model_path, "wb") as f:
        pickle.dump(model, f)
    logger.info(f"Saved trained HMM model to '{model_path}'.")

    timestamp_str = datetime.now().strftime("%Y%m%d_%H%M%S")
    versioned_path = f"model_{timestamp_str}.pkl"
    try:
    ...
    # 4. Save model selection history CSV (#8)
    if summary_df is not None:
        csv_path = "model_selection_history.csv"
        try:
